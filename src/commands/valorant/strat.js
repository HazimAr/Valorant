...
module.exports = class StratCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      name: "strategy",
      aliases: ["strat"],
      group: "valorant",
      memberName: "strategy",
      description: "Chooses a random valorant strategy",
    });
  }
  ...
  async run(message, args) {
    if (args.toLowerCase().startsWith("a")) {
      ...
      const randomStrategy =
        strategies[Math.floor(Math.random() * strategies.length)].join("\n");
      return message.say(randomStrategy);
    } else if (args.toLowerCase().startsWith("d")) {
      const strategies = [
        [
          "Is this CS:GO???",
          "No one on your team is allowed to use any abilities for this round. You might as well just be playing CS:GO.",
    ...
    message.say("Please choose a specified team ei (attacking or defending)");
  }
};
